...
public class TableIndexer {
 ...
 //           IndexWriterConfig indexConfig = new IndexWriterConfig(new StandardAnalyzer());  // Writer per scrivere sull'indice
            IndexWriterConfig indexConfig = new IndexWriterConfig(new WhitespaceAnalyzer());  // Writer per scrivere sull'indice
            ...
            while ((line = reader.readLine()) != null) {
                ...
                doc.add(new TextField("table", tableId, Field.Store.YES));   // Aggiungi l'id della tabella nel campo id

                /*SCORRI LA MAPPA E AGGIUNGI TUTTE LE COLONNE*/
                for (Map.Entry<String, StringBuilder> entry : columnData.entrySet()) {
                    if (entry.getKey() != null) {
                        doc.add(new TextField("column", entry.getKey().toString(), Field.Store.YES));
                    } else {
                        doc.add(new TextField("column", "empty", Field.Store.YES));
                    }
                    doc.add(new TextField("content", (entry.getValue().toString()), Field.Store.YES));
                }
